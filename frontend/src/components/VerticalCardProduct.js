...
const VerticalCardProduct = ({ category, heading }) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            console.error("Error fetching vertical product:", error)
        } finally {
            setLoading(false)
        }
    }, [category])
                            ...
                            return (
                                <Link to={"product/" + product?._id} className='w-full min-w-[280px]  md:min-w-[320px] max-w-[280px] md:max-w-[320px]  bg-white rounded-sm shadow '>
                                    ...
                                    <div className='p-4 grid gap-3'>
                                        ...
                                        <button className='text-sm bg-blue-600 hover:bg-blue-700 text-white py-1.5 rounded-xl' onClick={(e) => handleAddToCart(e, product?._id)}>Add to Cart</button>
                                    </div>
                                </Link>
                            )
                        })
