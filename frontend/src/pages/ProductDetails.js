      ...
      <div className='min-h-[200px] flex flex-col lg:flex-row gap-4'>
          ...
          <div className='flex flex-col gap-4 h-96 lg:flex-row-reverse'>
              ...
              <div className='h-[300px] w-[300px] lg:h-96 lg:w-96 bg-slate-200 relative p-2'>
                  <img src={activeImage} alt={data?.productName || 'Product image'} className='object-scale-down w-full h-full mix-blend-multiply' onMouseMove={handleZoomImage} onMouseLeave={handleLeaveImageZoom}/>
                          ...
                          data?.productImage?.map((imgURL,index) =>{
                            return(
                              <div className='w-20 h-20 p-1 rounded bg-slate-200' key={imgURL}>
                                <img src={imgURL} alt={`thumbnail ${index+1}`} className='object-scale-down w-full h-full cursor-pointer mix-blend-multiply' onMouseEnter={()=>handleMouseEnterProduct(imgURL)}  onClick={()=>handleMouseEnterProduct(imgURL)}/>
                              </div>
                            )
                          })
                        }
           ...
           {/***product details */}
           {
            loading ? (
              <div className='grid w-full gap-1'>
                ...
                <div className='flex items-center w-full h-6 gap-2 my-1 text-2xl font-medium lg:text-3xl lg:h-8 animate-pulse'>
                  <p className='w-full text-blue-600 bg-slate-200'></p>
            ...
            (
              <div className='flex flex-col gap-1'>
                <p className='inline-block px-2 text-blue-600 bg-blue-200 rounded-full w-fit'>{data?.brandName}</p>
